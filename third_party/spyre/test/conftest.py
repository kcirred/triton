...
def _parse_mlir_frontend(mlir_text: str):
    ...
    if proc.returncode != 0:
        raise RuntimeError(
            "MLIR parse worker failed. MLIRFrontendParser (mlir_ktdp) is "
            "required for numerical validation; install the bindings from "
            "the ktir-mlir-frontend submodule via the 'spyre-test' extra "
            f"(see setup.py).\n--- worker stderr ---\n{proc.stderr.decode()}"
        )
