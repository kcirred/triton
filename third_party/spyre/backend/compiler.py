...
class SpyreBackend(BaseBackend):
    ...
    def _make_ktir(self, mod, metadata, options):
        ...
        Pipeline (add_convert_ttir_to_ktdp expands to steps 1-3):
        1. LowerDescriptorMemory: tt.descriptor_load/store/gather/scatter -> ktdp.*
        2. LowerComputeOps: tt.reduce/broadcast/expand_dims -> linalg/tensor + dead op sweep
        3. ConvertFunctions: tt.func/return -> func.func/return, !tt.ptr -> index
           (must run last — memory passes consume !tt.ptr args via getBasePtrAsIndex)
        4. DistributeWork: tt.get_program_id -> ktdp.get_compute_tile_id
        5. canonicalize + CSE
        """
